...
contract TestRegistryStore {
    ...
    function testAdd() {
        Assert.isTrue(registryStore.add(bytes32("new.user"), address(0x1), address(0x2), bytes32('hash')) , 'It should store new record.');
        Assert.isFalse(registryStore.add(bytes32("new.user"), address(0x3), address(0x4), bytes32('hash')), 'It should not store(same id).');
        Assert.isFalse(registryStore.add(bytes32("new.user1"), address(0x1), address(0x4), bytes32('hash')), 'It should not store(same address).');
    }
